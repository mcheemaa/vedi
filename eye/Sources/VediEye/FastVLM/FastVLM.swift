    ...
    fileprivate class VisionModelCoreML {
        ...
        func load() throws -> fastvithd {
            try lock.withLock {
                ...
                // SPM has no app bundle to auto-load from; the captioner
                // compiles the mlpackage and provides the URL.
                let model: fastvithd
                if let url = FastVLM.visionModelURL {
                    model = try fastvithd(contentsOf: url)
                } else {
                    model = try fastvithd()
                }
                _model = model
...
public class FastVLM: Module, VLMModel, KVCacheDimensionProvider {
    ...
    /// Compiled fastvithd.mlmodelc location, set before first load.
    nonisolated(unsafe) public static var visionModelURL: URL?
